...
import type { PaymentListItem, PKSelectOption, PKRadioOption } from '../components'
...
function getPaymentHistoryParams({
  ...
  if (nextPayStatuses.includes('CANCEL_REQ')) {
    nextPayStatuses = nextPayStatuses.filter((status) => status !== 'CANCEL_REQ')
...
export const PaymentHistoryActivity: ActivityComponentType<'paymentHistory'> = () => {
  ...
  const yearWheelOptions = useMemo(
    () => yearOptions.map((option) => ({ label: option.label, value: Number(option.value) })),
    [yearOptions],
  )
  const monthWheelOptions = useMemo(
    () => monthOptions.map((option) => ({ label: option.label, value: Number(option.value) })),
    [],
  )

  const currentParams = useMemo(
    () => getPaymentHistoryParams({ filter, sort, page: currentPage.current }),
    [filter, sort],
  )
          ...
          <div className={classes.filterScroll}>
            ...
            <div className={classes.filterSection}>
              ...
              {modalFilter.selectrangeOption.includes('month') ? (
                <PKBottomWheelPicker
                  onChange={(value) =>
                    updateModalFilter('selectedMonth', value as PaymentHistoryFilter['selectedMonth'])
                  }
            ...
            <div className={classes.filterSection}>
              ...
              <PKRadioGroup
                onChange={(value) => updateModalFilter('payStatuses', value ?? [])}
...
const classes = {
  ...
  filterIcon: 'h-5 w-5',
  list:
    'grid min-h-0 flex-1 content-start gap-2 overflow-y-auto pt-4 pb-[env(safe-area-inset-bottom)] [-webkit-overflow-scrolling:touch]',
  listRefreshing: 'pointer-events-none opacity-50',
  loadingBox:
    'flex flex-col items-center justify-center gap-2 py-20',
  loadingText:
    'm-0 text-center text-[14px] leading-[1.4] text-[#8b9099]',
  empty:
